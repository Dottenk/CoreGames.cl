...
class MetacriticAPI
{
    ...
    public function getMetacriticScores()
    {
        ...
        if (!$error) {
            ...
            foreach ($html->find("div.metascore_w.user.large.game.positive") as $element) {
                $user_score = floatval($element->plaintext);
            }

            foreach ($html->find("div.metascore_w.user.large.game.negative") as $element) {
                $user_score = floatval($element->plaintext);
            }

            foreach ($html->find("div.metascore_w.user.large.game.mixed") as $element) {
                $user_score = floatval($element->plaintext);
            }
